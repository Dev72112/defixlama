...
export default function WhaleActivity() {
  ...
  const hhi = useMemo(() => {
    ...
    for (const p of protocolList) {
      const share = ((p.tvl || 0) / totalTvl) * 100;
      index += share * share;
    }
  ...
  const capitalVelocity = useMemo(() => {
    ...
    let totalFlow = 0;
    for (const p of protocolList) {
      const abs1d = Math.abs(p.change_1d || 0);
      totalFlow += (p.tvl || 0) * (abs1d / 100);
    }
  ...
  const whaleAlerts = useMemo(() => {
    return protocolList
      ...
      .map((p) => ({
        ...
        dollarMove: (p.tvl || 0) * (Math.abs(p.change_1d || 0) / 100),
      }));
  }, [protocolList]);
      ...
      <div className="space-y-6 page-enter">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Capital concentration, behavioral analytics, and large movement detection
          </p>
        </div>
        ...
        {/* TVL Concentration + Capital Distribution */}
              ...
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={topProtocols} layout="vertical" margin={{ left: 0, right: 16, top: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="share" radius={[0, 4, 4, 0]}>
                    {topProtocols.map((_, i) => (<Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            )}
                  ...
                  <div key={p.id || p.name} className={cn("flex items-center gap-3 p-3 rounded-lg border", SEVERITY_STYLES[p.severity])}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {p.category || "—"} • {formatCurrency(p.tvl)} • Est. move: {formatCurrency(p.dollarMove)}
                      </div>
                    </div>
